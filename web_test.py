import pandas as pd
import datetime
def test(name,column,percent,filename,save):
    print("Вхождение выполнено")
    tag_data = pd.read_excel('./input/kat.xlsx', skiprows=[0])
    head = pd.read_excel('./input/kat.xlsx', nrows=1)
    reve = pd.read_excel(filename)
    data = pd.DataFrame(columns=[name, 'категория', 'подкатегория', 'Дата'])
    ...
    for columns in ["Отзыв"]:
        for znach in reve[columns]:
            ...
            data = data._append({name: znach, 'категория': str(list(kt))[1:-1].replace("'", ''),
                                'подкатегория': str(list(pkt))[1:-1].replace("'", ''),
                                'Дата': str(reve.loc[reve['Отзыв'] == znach]['Дата создания отзыва'].values)[2:12]},
                               ignore_index=True)
            kt.clear()
            pkt.clear()
    ...
    for col_num, value in enumerate(data[name]):
        worksheet.write(col_num + 1, 0, value, header_format)
    worksheet.autofilter(f'B1:C1')
    worksheet.freeze_panes(1, 0)
    writer._save()
#test("hh","Отзыв",0.8,"./temp.xlsx","./static/fin.xlsx")
